...
module Make(H : HASHABLE) : SET with type elt = H.t = struct
  ...
  let empty =
    { tbl = Array.make initial_capacity Empty; size = 0; used = 0 }
    ...
    let rec loop i first_tomb =
      if i = cap then
        ...
        | Empty ->
            ...
            (match first_tomb with Some j -> `Insert j | None -> `Insert idx)
        | Tombstone ->
  ...
  let load_factor_used s =
    float_of_int s.used /. float_of_int (capacity s)

  let next_pow2 n =
    let n = if n < 8 then 8 else n in
    let rec up k =  (* multiply k by 2 until k >= n *)
      if k >= n then k
      else up (k lsl 1)   
    in
    up 1
  ...
  let rehash new_cap s =
    ...
    in
    Array.iter (function
      | Occupied x -> insert_raw x
      | _ -> ()
    ) s.tbl;
    { tbl = tbl'; size = s.size; used = s.size }

  let ensure_room s =
    if load_factor_used s > 0.7 then
      rehash (capacity s lsl 1) s
    else s
  ...
  let remove x s =
    ...
    | `Found idx ->
        ...
        if capacity s' > initial_capacity
  ...
  let iter f s =
    Array.iter (function Occupied x -> f x | _ -> ()) s.tbl

  let fold_left f acc s =
    let r = ref acc in
    iter (fun x -> r := f !r x) s; !r
  ...
  let subset a b =
    fold_left (fun ok x -> ok && mem x b) true a

  let to_list s =
    fold_right (fun x acc -> x :: acc) s []
