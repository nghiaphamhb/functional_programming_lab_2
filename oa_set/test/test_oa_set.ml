...
let to_set xs =
  List.fold_left (fun acc x -> SInt.add x acc) SInt.empty xs
...
let test_union_equal () =
  ...
  Alcotest.(check bool) "identity" true (SInt.equal_set (SInt.union SInt.empty a) a)
...
let prop_monoid_assoc =
  Test.make
    ~count:300
    ~name:"monoid_assoc"
    (triple gen_int_list gen_int_list gen_int_list)
    (fun (xs, ys, zs) ->
...
let check_prop (t : QCheck.Test.t) () =
